...
Every Full Conversion and Update/Fold-in also produces **Mermaid diagrams** so the skill can be navigated visually, not just linearly:

- A dedicated `diagrams.md` file with a book mind-map, framework relationship flowchart, decision tree, and concept dependency graph.
- An optional inline `## Visual` Mermaid block inside each chapter file (omitted when a diagram would add no signal).
- A high-level "Visual Map" section near the top of `SKILL.md` for a single-glance overview.
- Optionally, rendered PNGs in a `visuals/` folder when `mermaid-cli` (`mmdc`) is installed — otherwise the Mermaid blocks render natively in GitHub, VS Code, and Claude Code markdown preview.

Visuals are generated by the agent following the steps below; no extra runtime dependency is required for the default Mermaid-only path.
...
## Visual *(optional — omit if a diagram adds no signal)*
<!-- One small Mermaid block (max ~15 lines) summarizing this chapter's core framework,
     flow, or decision. Reference chNN in node labels so it doubles as navigation.
     Omit entirely for purely conceptual chapters where a diagram would be noise. -->
```mermaid
flowchart TD
    A[Concept from this chapter] --> B[Outcome / next chapter]
```
...
## Step 7.5 — Generate visual diagrams

Create `$SKILLS_HOME/<skill_name>/diagrams.md` containing Mermaid diagrams that let the user (and the agent) navigate the book visually. Visuals complement the linear text — they are not a substitute for the chapter summaries.

**TOKEN BUDGET: Keep the whole `diagrams.md` under 2,500 tokens.** Density over completeness.

Start the file with a one-line "How to view" note:

```markdown
# Visual Map — <Book Title>

> Mermaid blocks render natively in GitHub, VS Code, and Claude Code preview.
> For PNGs, see `visuals/` (generated only when `mmdc` is installed).
```

Then emit **4–6** of the diagrams below. **Skip any diagram type that does not apply to the book** (e.g. omit `sequenceDiagram` for a book with no multi-step processes).

| Diagram | Mermaid type | When to include |
|---|---|---|
| Book structure map | `mindmap` | Always. Root = book title; branches = parts/chapters; leaves = 2–4 key frameworks per chapter. Single-glance overview. |
| Framework relationship flowchart | `flowchart TD` | Always (when ≥2 named frameworks exist). Shows how the author's frameworks connect, depend on, or feed into each other. |
| Decision flowchart | `flowchart TD` | When `cheatsheet.md` has decision rules. "Use framework X when Y" rendered as a decision tree — the visual cheatsheet. |
| Process / pipeline sequence | `sequenceDiagram` | Only when the book describes multi-step processes (pipelines, request flows, lifecycles). |
| Concept dependency graph | `graph LR` | Always. Which concepts must be understood before others — the recommended learning path. |
| State / phase transitions | `stateDiagram-v2` | Only when the book models discrete states (e.g. system states, maturity phases). |

**Rules:**
- Reference real chapter numbers (`chNN`) as node labels so every diagram doubles as navigation.
- Add a one-line caption above each block: what it shows + when to consult it.
- Preserve the author's exact framework names inside nodes.
- Example scaffolds:
...
```mermaid
flowchart TD
    A[ch01 Framework A] --> B[ch02 Framework B]
    B --> C{Decision: when?}
    C -->|case X| D[ch03 Framework D]
    C -->|case Y| E[ch04 Alternative]
```

```mermaid
graph LR
    A[ch01 Basics] --> B[ch03 Advanced topic]
    A --> C[ch02 Prerequisite concept]
    C --> B
```
...
<!-- One high-level mindmap: the book at a glance. Front-loaded so the first load
     gives a visual overview. Keep under ~40 lines. -->
```mermaid
mindmap
  root((<Book Title>))
    Part I
...
> For the full set of flowcharts, decision trees, and the concept dependency graph, open [diagrams.md](diagrams.md).
...
- [diagrams.md](diagrams.md) — Mermaid mind-map, flowcharts, decision tree, and concept dependency graph
...
### Optional — render Mermaid blocks to PNG

Only run this if `mmdc` (mermaid-cli) is installed. If absent, skip silently — the Mermaid blocks render natively in GitHub, VS Code, and Claude Code preview, so the skill is fully usable without PNGs.

```bash
SKILL_DIR="$SKILLS_HOME/<skill_name>"
if command -v mmdc >/dev/null 2>&1; then
  mkdir -p "$SKILL_DIR/visuals"
  # Render the master diagrams file and each chapter with an inline mermaid block.
  mmdc -i "$SKILL_DIR/diagrams.md" -o "$SKILL_DIR/visuals/diagrams.png" -b transparent
  for f in "$SKILL_DIR"/chapters/*.md; do
    [ -f "$f" ] || continue
    grep -q '```mermaid' "$f" || continue
    name=$(basename "$f" .md)
    mmdc -i "$f" -o "$SKILL_DIR/visuals/${name}.png" -b transparent 2>/dev/null || true
  done
  echo "Rendered PNGs -> $SKILL_DIR/visuals/"
else
  echo "mmdc not installed — Mermaid blocks render natively in GitHub/VS Code."
  echo "To render PNGs: npm i -g @mermaid-js/mermaid-cli"
fi
```
...
Files generated:
  SKILL.md         — core frameworks + visual map + index   (~X tokens)
  chapters/        — <N> chapter summaries + inline visuals (~X tokens each, ~X total)
  diagrams.md      — Mermaid mind-map, flowcharts, decision tree   (~2,500 tokens)
  glossary.md      — key terms                 (~X tokens)
  patterns.md      — techniques & patterns     (~X tokens)
  cheatsheet.md    — quick reference           (~X tokens)
  visuals/         — rendered PNGs (only if mmdc installed; otherwise omitted)
  ─────────────────────────────────────────────────────────────────
...
### 4.5. Regenerate Visual Diagrams
- Read the existing `$SKILLS_HOME/<skill_name>/diagrams.md` (if present).
- Regenerate the **framework relationship flowchart** and **concept dependency graph** to include the new chapters (append new `chNN` nodes; wire them to related existing frameworks).
- Update the **book structure mind-map** to add the new chapter branches.
- For each new or revised chapter file, add an inline `## Visual` Mermaid block per the Step 7 template (omit if a diagram adds no signal).
- Keep the total `diagrams.md` under 2,500 tokens; prune lower-signal nodes if needed.
- If `diagrams.md` did not previously exist (older skill), create it now from scratch following Step 7.5.
